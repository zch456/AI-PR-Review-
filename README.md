...
当前阶段实现了项目骨架、GitHub PR URL 解析和 PR 元信息获取闭环：前端提交 PR 链接，后端解析出仓库 owner、仓库名和 PR 编号，并从 GitHub API 获取 PR 标题、作者、状态、分支、增删行和变更文件数量。
...
- [开发计划](docs/开发计划.md)：项目目标、架构思路、已完成阶段、下一步迭代和日常开发流程。
- [踩坑记录](docs/踩坑记录.md)：开发中值得复盘的问题和后续约束。
...
- 在页面展示 owner、repo、PR 编号、标题、作者、状态、分支、增删行和变更文件数量。
...
当前版本使用 GitHub 公开 REST API 获取 PR 元信息。分析公开仓库 PR 时不需要额外配置 token。后续如果要支持私有仓库或更高频率请求，需要增加 GitHub token 配置。
