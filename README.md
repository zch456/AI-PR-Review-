...
- [开发历程](docs/开发历程.md)：回顾项目从最小骨架到 AI Review 闭环的演进过程。
- [开发计划](docs/开发计划.md)：记录当前架构、近期计划和中期路线。
- [踩坑记录](docs/踩坑记录.md)：沉淀路径、端口、PR 流程、密钥、误报等问题。
- [优化拓展](docs/优化拓展.md)：面向下一阶段的产品、工程、算法和体验升级方案。
...
- 当前只支持 `github.com` 的 PR 链接。
- 默认只适合公开仓库；私有仓库需要后续增加 GitHub token 支持。
- AI Review 依赖 DeepSeek API Key，没有配置时只能完成 PR 信息获取和规则风险预检查。
- 大 PR 目前按字符上限截断上下文，后续需要做分块分析和聚合。
- 规则风险预检查仍是启发式规则，可能存在误报或漏报。
